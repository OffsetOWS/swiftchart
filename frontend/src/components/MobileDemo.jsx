    ...
    <div className="graphite-screen">
      ...
      <div className="graphite-plans">
        <article>
          ...
          {activePlan === "Pro Monthly" || activePlan === "Pro Lifetime" ? <em>Active</em> : <button type="button" onClick={() => onPayment("pro_monthly")}>Pay with USDC</button>}
        </article>
        <article>
          <span>Pro Lifetime</span>
          <strong>$99.99 one-time</strong>
          <p>Lifetime beta access, all Pro features, Early Supporter badge.</p>
          {activePlan === "Pro Lifetime" ? <em>Active</em> : <button type="button" onClick={() => onPayment("pro_lifetime")}>Pay with USDC</button>}
        </article>
      </div>
    </div>
  );
...
export default function MobileDemo({ topIdeas = [], initialConfigLoading = false, initialTab = "" }) {
  ...
  useEffect(() => {
    if (accountView !== "payment" || auth.loading || auth.isAuthenticated) return;
